...
class _HomePageState extends State<HomePage> {
  ...
  ChatShowCount() async {
    ...
    checkInternet().then((internet) async {
      if (internet) {
        HomeProvider().chatCountApi(bodyData).then((
            response,
            ) async {
  ...
  ParcelShowCount() async {
    ...
    checkInternet().then((internet) async {
      if (internet) {
        HomeProvider().parcelShowCountApi(bodyData).then((
            response,
            ) async {
          parcelShowCountModel = ParcelShowCountModel.fromJson(response.data);
  ...
  VisitorShowCount() async {
    ...
    checkInternet().then((internet) async {
      if (internet) {
        HomeProvider().visitorShowCountApi(bodyData).then((
            response,
            ) async {
          visitorShowCountModel = VisitorShowCountModel.fromJson(response.data);
...
//
//     // Initial API calls
//     _makeInitialApiCalls();
//
//     // Start timer with longer interval
//     _timer = Timer.periodic(const Duration(seconds: 30), (timer) {
//       if (_canMakeApiCall('periodic_update')) {
//         _fetchCountData();
//       }
//     });
//   }
//
//   void _makeInitialApiCalls() {
//     // Make initial API calls without delay
//     VisitorShowCount();
//     ParcelShowCount();
//     ChatShowCount();
//     MessageBoardApi();
...
//   }
//
//   void _fetchCountData() {
//     // Fetch count data with rate limiting
//     if (_canMakeApiCall('count_update')) {
//       VisitorShowCount();
//     }
//
...
//   }
//
//   bool _canMakeApiCall(String apiType) {
//     // Prevent multiple simultaneous API calls
//     if (_isApiCallInProgress) return false;
//
//     // Rate limiting: max 1 call per 10 seconds per API type
//     final now = DateTime.now();
//     final lastCallTime = _lastApiCallTime;
//
...
//       if (_canMakeApiCall('app_resume')) {
//         _fetchCountData();
//       }
//     } else if (state == AppLifecycleState.paused) {
//       // App went to background - cancel ongoing API calls
...
//   }
//
//   // ... (Rest of your methods remain the same until the API methods)
//
//   void ParcelShowCount() {
//     if (!_canMakeApiCall('parcel_count')) return;
//
...
//     if (!_canMakeApiCall('visitor_count')) return;
//
//     _isApiCallInProgress = true;
//     _updateApiCallTime();
//
...
//
//   void ChatShowCount() {
//     if (!_canMakeApiCall('chat_count')) return;
//
//     _isApiCallInProgress = true;
//     _updateApiCallTime();
//
...
//     if (!_canMakeApiCall('message_board')) return;
//
//     _isApiCallInProgress = true;
//     _updateApiCallTime();
//
